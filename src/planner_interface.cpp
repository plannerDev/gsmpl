...
{
    auto samplerParam = std::static_pointer_cast<const SampleWithBiasParam>(context.samplerParam);
    si_ = std::make_shared<SpaceInformation>(context.generalParam, *samplerParam, pd_,
...
bool PlannerInterface::allocPlanner(const PlannerContext& context)
{
    ...
    {
        allocSi(context);
        planner_ = std::make_shared<RRTStar>(si_, pd_, context, vf_);
        return true;
    }
    case PlannerType::InformedRRTStar:
    {
        allocSi(context);
        planner_ = std::make_shared<InformedRRTStar>(si_, pd_, context, vf_, vpf_);
        return true;
    }
    }
    return false;
...
bool PlannerInterface::plan(PlannerSolution& solution)
{
    ...
    if (solution.isValid()) {
        ...
        pt.planner = std::chrono::duration<float>(endTime3 - startTime1).count();
        pt.rrtSolver = std::chrono::duration<float>(endTime1 - startTime1).count();
        pt.pathProcessing = std::chrono::duration<float>(endTime2 - startTime2).count();
        pt.print();
        ...
        record_.addPath(simplifierSolution.simplified_2.waypoints, "simplified_2");
        vf_(record_);
        return true;
    }
