...
RRTStar::RRTStar(const SpaceInformationBasePtr& si, const ProblemDefinition& pd,
    ...
    : Planner(si, pd), param_(*param(context)), vf_(vf),
...
const PlannerSolution& RRTStar::solve()
{
    ...
    for (; i < param_.steps; ++i) {
        ...
        double r = std::max(
            param_.stepSize,
            r_rrt * std::pow(log(cardDbl) / cardDbl, 1 / static_cast<double>(si_->dimension)));
        r = 0.5;
        ...
        if (isValidEdge(vNearest->state, xNew)) {
            std::vector<VertexPtr> vNearK = nearestNeighbor_->nearestR(xNew, r); // nearestK
            std::cout << "R_rrt " << r_rrt << " cardDbl " << cardDbl << " vNearK " << vNearK.size()
                      << " r " << r << std::endl;
            ...
            if (!haveSolution && goal_->isSatisfied(vNew->state)) {
                ...
                if (isValidEdge(vNew->state, currentGoal_)) {
                    ...
                    const VertexPtr vGoal =
                        std::make_shared<Vertex>(vNew.get(), currentGoal_, sCost, eCost);
...
State RRTStar::steer(const State& qSampled, const State& qNear, double stepSize) const
{
