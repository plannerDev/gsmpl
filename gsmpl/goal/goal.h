...
public:
    ...
    bool isSatisfied(const State& q) const override {
        return distance_->isEquivalent(q, q_);
    }
...
//     {
//         Eigen::Quaterniond v(0.0, v_[0], v_[1], v_[2]); // v: unit vector, q:
//         versor auto qTemp = q * v * q.inverse(); Eigen::Vector3d
//         vRotate{qTemp.x(), qTemp.y(), qTemp.z()};
...
//     {
//         return pose[0] >= center_[0] - region_[0] && pose[0] <= center_[0] +
//         region_[0] &&
//                pose[1] >= center_[1] - region_[1] && pose[1] <= center_[1] +
//                region_[1] && pose[2] >= center_[2] - region_[2] && pose[2] <=
//                center_[2] + region_[2];
//     }
...
//         const Eigen::Vector3d& v, double angle) : box_(center, region),
//         quat_(v, angle) { }
