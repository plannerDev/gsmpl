...
Path PathSimplifier::reduceVertices(const Path& rawPath, double step_size_jps,
        ...
        for (unsigned int i = 0; i < newPath.size(); i++) {
            ...
            if (p1 >= 0 && p2 >= 0) {
                if (localPlanner_->validInterpolatePath(
                        newPath[p1], newPath[p2], step_size_jps, stepSizeTcp)) {
        ...
        while (i < length) {
            ...
            if (temp1 && temp2) {
                if (auto qNew = localPlanner_->validInterpolateState(
                    ...
                    if (localPlanner_->validInterpolatePath(
                            newPath[i - 1], qNew.value(), step_size_jps,
                            stepSizeTcp) &&
                        localPlanner_->validInterpolatePath(
                            qNew.value(), newPath[i + 1], step_size_jps,
                            stepSizeTcp)) {
