...
{
public:
    LocalPlannerBase(const DistanceBasePtr& distance, const StateCheckerBasePtr& checker)
        : dimension(distance->dimension), distance_(distance), checker_(checker)
    {
    }
    virtual ~LocalPlannerBase() = default;

    // interpolate without validity check
    virtual State interpolateState(const State& q1, const State& q2, double time) const = 0;
    virtual std::optional<State> validInterpolateState(const State& q1, const State& q2,
                                                       double time) const = 0;
    // time[0.0, 1.0], return true, if interpolated path is valid
    virtual std::optional<Path> validInterpolatePath(const State& q1, const State& q2,
                                                     double jpsStepSize,
                                                     double tcpStepSize) const = 0;
    virtual Path interpolatePath(const State& q1, const State& q2, double jpsStepSize,
                                 double tcpStepSize) const = 0;
...
{
public:
    JointInterpolateSimple(const DistanceBasePtr& distance, const StateCheckerBasePtr& checker)
