...
Path PathSimplifier::reduceVertices(const Path& rawPath, double stepSizeJps, double stepSizeTcp,
                                    unsigned int maxSteps, unsigned int maxEmptySteps,
                                    double rangeRatio)
{
    if (rawPath.size() < 3)
        return rawPath;
    ...
    for (std::size_t i = 0; i < maxSteps && emptySteps < maxEmptySteps; i++, emptySteps++) {
        ...
        int maxN = count - 1;
        int range = 1 + static_cast<int>(floor(0.5 + static_cast<double>(count) * rangeRatio));
        ...
        if (p1 > p2)
            std::swap(p1, p2);
        if (localPlanner_->validInterpolatePath(newPath[p1], newPath[p2], stepSizeJps,
                                                stepSizeTcp)) {
...
Path PathSimplifier::collapseCloseVertices(const Path& rawPath, double stepSizeJps,
...
Path PathSimplifier::smoothBSpline(const Path& rawPath, unsigned int maxSteps, double minChange,
                                   double stepSizeJps, double stepSizeTcp) const
{
    ...
    for (unsigned int step = 0; step < maxSteps; step++) {
        ...
        while (i < length) {
            ...
            if (temp1 && temp2) {
                if (auto qNew =
                        localPlanner_->validInterpolateState(temp1.value(), temp2.value(), 0.5)) {
                    if (localPlanner_->validInterpolatePath(newPath[i - 1], qNew.value(),
                                                            stepSizeJps, stepSizeTcp) &&
                        localPlanner_->validInterpolatePath(qNew.value(), newPath[i + 1],
                                                            stepSizeJps, stepSizeTcp)) {
...
Trajectory PathSimplifier::generateTrajector(const Path& path, double stepSizeJps,
                                             double stepSizeTcp) const
{
    ...
    for (int i = 1; i < path.size(); i++) {
        ...
        segment = localPlanner_->interpolatePath(path[i - 1], path[i], stepSizeJps, stepSizeTcp);
